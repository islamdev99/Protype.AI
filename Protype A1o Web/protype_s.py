...
def perform_search(query):
    try:
        params = {
            "engine": "google",
            "q": query,
            "api_key": "f41082ce8546f83f717679baf1318d649d123ba213a92067de9dfdee2ea5accb"  # Replace with your key
        }
        search = serpapi.Client(params)
...
# Function to implement deep thinking and analysis
def deep_thinking_analysis(question):
    """
    Perform deep analysis on a question regardless of the available AI services
    """
    ...
    # Generate a thoughtful analysis
    analysis = f"Deep analysis of '{question}':\n"
    analysis += f"- Question type: {question_type}\n"
    analysis += f"- Detected entities: {', '.join(entities) if entities else 'No specific entities detected'}\n"
    analysis += f"- Key concepts: {', '.join(key_phrases[:5]) if key_phrases else 'No key concepts identified'}\n\n"
    
    # Consider semantic context and knowledge graph
    related_concepts = []
    for term in key_phrases + entities:
        if term in globals()['knowledge_graph'].nodes:
            neighbors = list(globals()['knowledge_graph'].neighbors(term))
            if neighbors:
                related_concepts.extend(neighbors[:3])
    
    if related_concepts:
        analysis += f"- Related concepts from knowledge graph: {', '.join(related_concepts[:5])}\n\n"
...
def query_gemini_flash(question):
    try:
        # First perform deep thinking analysis
        deep_analysis = deep_thinking_analysis(question)
        
        # Add deep thinking prompt to enhance Gemini's response
        enhanced_prompt = f"""
        As Protype.AI developed by Islam Ibrahim:
        
        I need to provide a thoughtful, well-reasoned response about: "{question}"
        
        My initial analysis:
        {deep_analysis}
        
        Based on this analysis, please provide a comprehensive, detailed response.
        If the question is in Arabic, please respond in Arabic.
        """
        ...
        try:
            ...
            # Fallback to our own deep thinking if Gemini fails
            analysis = deep_analysis
            
            # Add a direct response based on our analysis
            analysis += "Based on my analysis, I can provide the following insights:\n\n"
            
            # Check for relevant information in our knowledge graph
            data = load_data()
            relevant_info = []
            
            for known_q in data.keys():
                doc1 = nlp(question.lower())
                doc2 = nlp(known_q.lower())
                if doc1.similarity(doc2) > 0.6:  # Threshold for relevance
                    relevant_info.append((known_q, data[known_q][0]["answer"], doc1.similarity(doc2)))
            
            # Sort by relevance
            relevant_info.sort(key=lambda x: x[2], reverse=True)
            
            if relevant_info:
                analysis += "I found some relevant information in my knowledge base:\n\n"
                for q, a, _ in relevant_info[:3]:
                    analysis += f"Related to '{q}':\n{a}\n\n"
            else:
                analysis += "I don't have specific information about this in my knowledge base yet, but I can make some logical inferences based on my analysis.\n\n"
            
            # Check if question is in Arabic
            if any('\u0600' <= c <= '\u06FF' for c in question):
                analysis += "أود أن أقدم المزيد من المعلومات عندما تتاح لي. سأواصل التعلم حول هذا الموضوع."
            else:
                analysis += "I would like to provide more information when available. I'll continue learning about this topic."
            
            return analysis
            
    except Exception as e:
        print(f"Error in deep thinking: {e}")
        return f"I encountered an error while thinking deeply about this question: {e}"
...
class ProtypeE0App:
    ...
    def setup_chat_tab(self):
        ...
        self.chat_output.insert(
            tk.END, "Hello! I'm Protype.AI, developed by Islam Ibrahim at Carrot Studio. Ask me anything!\n")
        self.chat_output.insert(
            tk.END, "مرحباً! أنا بروتايب، تم تطويري بواسطة إسلام إبراهيم في كاروت ستوديو. اسألني أي شيء!\n")
