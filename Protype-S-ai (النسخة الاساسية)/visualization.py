...
def create_knowledge_graph_viz(knowledge_graph, max_nodes=50, highlight_inferred=True):
    ...
    try:
        ...
        # Separate normal and inferred edges
        normal_edges = []
        inferred_edges = []
        ...
        # Draw normal edges
        nx.draw_networkx_edges(graph, pos, edgelist=normal_edges, alpha=0.5, arrows=True)
        
        # Draw inferred edges with different style if requested
        if highlight_inferred and inferred_edges:
            nx.draw_networkx_edges(graph, pos, edgelist=inferred_edges, 
                                  alpha=0.8, arrows=True, 
                                  edge_color='orange', style='dashed', width=2)
        ...
        # Add legend for inferred relationships
        if highlight_inferred and inferred_edges:
            from matplotlib.lines import Line2D
            legend_elements = [
                Line2D([0], [0], color='black', alpha=0.5, label='Explicit Relationship'),
                Line2D([0], [0], color='orange', alpha=0.8, linestyle='dashed', label='Inferred Relationship')
            ]
            ax.legend(handles=legend_elements, loc='upper left')

        inferred_count = len(inferred_edges) if highlight_inferred else 0
        ax.set_title(f"Knowledge Graph ({len(graph.nodes)} nodes, {inferred_count} inferred connections)")
        ax.axis('off')
