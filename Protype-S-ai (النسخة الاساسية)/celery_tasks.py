...
def train_knowledge_gnn():
    """Task to train the GNN model on the current knowledge graph"""
    try:
        # Import here to avoid circular imports
        import web_app
        from graph_neural_network import KnowledgeGNN
        
        # Get the current knowledge graph
        if not web_app.knowledge_graph or len(web_app.knowledge_graph.nodes) < 5:
            return {"status": "error", "reason": "Knowledge graph too small for GNN training"}
        
        # Initialize GNN
        gnn = KnowledgeGNN(web_app.knowledge_graph)
        gnn.build_model()
        
        # Train model
        if gnn.train(epochs=100):
            # Infer new relationships
            inferred_edges = gnn.infer_new_knowledge(threshold=0.65)
            
            # Update the global GNN model
            web_app.knowledge_gnn = gnn
            
            # Add inferred edges to the graph (limit to avoid noise)
            added_edges = 0
            for source, target, score in inferred_edges[:30]:
                if not web_app.knowledge_graph.has_edge(source, target):
                    web_app.knowledge_graph.add_edge(source, target, 
                                                   relation="inferred", 
                                                   confidence=score,
                                                   inferred=True)
                    added_edges += 1
            ...
            return {
                "status": "success", 
                "nodes": len(web_app.knowledge_graph.nodes),
                "inferred_relationships": added_edges,
                "total_inferred": len(inferred_edges)
            }
        
        return {"status": "error", "reason": "GNN training failed"}
    except Exception as e:
