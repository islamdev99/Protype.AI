...
# تكوين للمؤثرات الصوتية
app.config['SOUND_EFFECTS_API_KEY'] = os.environ.get('ELEVEN_LABS_API_KEY', 'أضف_مفتاحك_الخاص')
...
@app.route('/sound-effects')
def sound_effects():
    """صفحة مولد المؤثرات الصوتية"""
    return render_template('sound_effects.html')

@app.route('/chat', methods=['POST'])
def chat():
    question = request.json.get('question', '').strip()
    apply_thinking = request.json.get('apply_thinking', False)
    ...
    if search_results:
        ...
        # Apply deep thinking only if requested
        if apply_thinking:
            final_answer = apply_deep_thinking(question, answer, question_type)
        else:
            final_answer = answer
        ...
        return jsonify({
            "status": "success", 
            "answer": final_answer,
            "raw_answer": answer,  # إضافة الإجابة الأصلية
            "source": result[3],
            "question_type": question_type,
            "thinking_applied": apply_thinking
        })
    else:
...
# تسجيل واجهة برمجة المؤثرات الصوتية
from app.api.sound_effects import register_sound_api
register_sound_api(app)
